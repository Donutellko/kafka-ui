...
public class ReactiveAdminClient implements Closeable {
  ...
  // NOTE: if KafkaFuture returns null, that Mono will be empty(!), since Reactor
  // does not support nullable results
  ...
  public static <T> Mono<T> toMono(KafkaFuture<T> future) {
    return Mono.<T>create(sink -> future.whenComplete((res, ex) -> {
      if (ex != null) {
        ...
        // (from docs it should be ExecutionException, be we actually see
        // CompletionException, so checking both
        ...
        // and by default all downstream operations (like map(..)) on created Mono will
  ...
  // NOTE: skips not-found topics (for which UnknownTopicOrPartitionException was
  // thrown by AdminClient)
  // and topics for which DESCRIBE_CONFIGS permission is not set
  // (TopicAuthorizationException was thrown)
  public Mono<Map<String, List<ConfigEntry>>> getTopicsConfig(Collection<String> topicNames, boolean includeDoc) {
    ...
    // we need to partition calls, because it can lead to AdminClient timeouts in
    // case of large topics count
  ...
  private Mono<Map<String, List<ConfigEntry>>> getTopicsConfigImpl(Collection<String> topicNames, boolean includeDoc) {
    ...
    return toMonoWithExceptionFilter(
        ...
        TopicAuthorizationException.class).map(
  ...
  private static Mono<Map<Integer, List<ConfigEntry>>> loadBrokersConfig(AdminClient client, List<Integer> brokerIds) {
    ...
    return toMono(client.describeConfigs(resources).all())
        ...
        // there are situations when kafka-ui user has no DESCRIBE_CONFIGS permission on
        // cluster
  ...
  /**
   * Return per-broker configs or empty map if broker's configs retrieval not
   * supported.
   */
  ...
  private Mono<Map<String, TopicDescription>> describeTopicsImpl(Collection<String> topics) {
    return toMonoWithExceptionFilter(
        ...
        UnknownTopicOrPartitionException.class,
        // we only describe topics that we see from listTopics() API, so we should have
        // permission to do it,
        // but also adding this exception here for rare case when access restricted
        // after we called listTopics()
  ...
  /**
   * Kafka API often returns Map responses with KafkaFuture values. If we do
   * allOf()
   * logic resulting Mono will be failing if any of Futures finished with error.
   * In some situations it is not what we want, ex. we call describeTopics(List
   * names) method and
   * we getting UnknownTopicOrPartitionException for unknown topics and we what to
   * just not put
   * such topics in resulting map.
   * <p/>
   * This method converts input map into Mono[Map] ignoring keys for which
   * KafkaFutures
  ...
  private static Mono<ClusterDescription> describeClusterImpl(AdminClient client, Set<SupportedFeature> features) {
    boolean includeAuthorizedOperations = features
        .contains(SupportedFeature.DESCRIBE_CLUSTER_INCLUDE_AUTHORIZED_OPERATIONS);
    ...
    return toMono(allOfFuture).then(
        Mono.fromCallable(() -> new ClusterDescription(
  ...
  // NOTE: places whole current topic config with new one. Entries that were
  // present in old config,
  ...
  public Mono<Table<String, TopicPartition, Long>> listConsumerGroupOffsets(List<String> consumerGroups,
    ...
    Function<Collection<String>, Mono<Map<String, Map<TopicPartition, OffsetAndMetadata>>>> call = groups -> toMono(
    ...
    return merged.map(map -> {
      ...
      map.forEach((g, tpOffsets) -> tpOffsets.forEach((tp, offset) -> {
        if (offset != null) {
          // offset will be null for partitions that don't have committed offset for this
          // group
   ...
   *
   * @param failOnUnknownLeader true - throw exception in case of no-leader
   *                            partitions,
  ...
  private Mono<Collection<TopicPartition>> filterPartitionsWithLeaderCheck(Collection<TopicPartition> partitions,
    ...
    return describeTopicsImpl(targetTopics)
        .map(descriptions -> filterPartitionsWithLeaderCheck(
      ...
      for (TopicPartitionInfo partitionInfo : description.partitions()) {
        ...
        if (partitionInfo.leader() == null) {
          if (failOnUnknownLeader) {
            ...
            // if ANY of topic partitions has no leader - we have to skip all topic
            // partitions
  ...
  // 1. NOTE(!): should only apply for partitions from topics where all partitions
  // have leaders,
  // otherwise AdminClient will try to fetch topic metadata, fail and retry
  ...
  // 3. TODO: check if it is a bug that AdminClient never throws
  // LeaderNotAvailableException and just retrying instead
  ...
  Mono<Map<TopicPartition, Long>> listOffsetsUnsafe(Collection<TopicPartition> partitions, OffsetSpec offsetSpec) {
    ...
    Function<Collection<TopicPartition>, Mono<Map<TopicPartition, Long>>> call = parts -> {
  ...
  /**
   * Splits input collection into batches, converts each batch into Mono,
   * sequentially subscribes to them
  ...
  /**
   * Splits input collection into batches, converts each batch into Mono,
   * subscribes to them (concurrently,
